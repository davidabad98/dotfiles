#!/usr/bin/env bash
# only run the rest of this file for interactive shells
[[ $- != *i* ]] && return
...
# only run fastfetch if /usr/bin/fastfetch exists AND we're NOT inside tmux
if [ -z "$TMUX" ] && [ -f /usr/bin/fastfetch ]; then
    /usr/bin/fastfetch
fi
