...
permission:
  ...
  bash:
    ...
    # Staging is limited to the approved candidate and requires confirmation.
    "git add*": ask
    # Restore can discard user work and always requires confirmation.
    "git restore*": ask
...
- If already provided in the request, stage only the explicitly identified
  candidate paths and commit. Never use `git add -A`, `git add --all`, or a
  broad pathspec; stop and ask if the intended staging set is not explicit.
- Never push.
